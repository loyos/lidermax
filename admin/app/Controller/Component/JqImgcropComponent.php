...
class JqImgcropComponent extends Object {
    ...
    function uploadImage($uploadedInfo, $uploadTo, $prefix){
        $webpath = $uploadTo;
		$uploadedInfo["name"] = $uploadedInfo["name"];
        $upload_dir = WWW_ROOT.str_replace("/", DS, $uploadTo);
        $upload_path = '/home/ingenili/public_html/app/webroot/';
        $max_file = "34457280";                         // Approx 30MB
        $max_width = 5000;
		//debug.die( $uploadedInfo["tmp_name"]);
        ...
        $uploadTarget = $upload_path.$uploadTo.$filename;
		//debug.die($uploadTarget);
